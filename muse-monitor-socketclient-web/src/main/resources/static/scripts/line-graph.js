 ...
 * 
 * Arguments: containerId => id of container to insert SVG into [REQUIRED]
 ...
 * data => a dictionary containing the following keys [REQUIRED] values => The
 * data array of arrays to graph. [REQUIRED] start => The start time in
 * milliseconds since epoch of the data. [REQUIRED] end => The end time in
 * milliseconds since epoch of the data. [REQUIRED] step => The time in
 * milliseconds between each data value. [REQUIRED] names => The metric name for
 * each array of data. [REQUIRED] displayNames => Display name for each metric.
 * [OPTIONAL => Default: same as 'names' argument] Example: ['MetricA',
 * 'MetricB'] axis => Which axis (left/right) to put each metric on. [OPTIONAL =>
 * Default: Display all values on single axis] Example: ['left', 'right',
 * 'right'] to display first metric on left axis, next two on right axis. colors =>
 * What color to use for each metric. [OPTIONAL => Default: black] Example:
 * ['blue', 'red'] to display first metric in blue and second in red. scale =>
 * What scale to display the graph with. [OPTIONAL => Default: linear] Possible
 * Values: linear, pow, log rounding => How many decimal points to round each
 * metric to. [OPTIONAL => Default: Numbers are rounded to whole numbers (0
 * decimals)] Example: [2, 1] to display first metric with 2 decimals and second
 * metric with 1. numAxisLabelsPowerScale => Hint for how many labels should be
 * displayed for the Y-axis in Power scale. [OPTIONAL => Default: 6]
 * numAxisLabelsLinearScale => Hint for how many labels should be displayed for
 * the Y-axis in Linear scale. [OPTIONAL => Default: 6]
 * 
 * Events (fired from container): LineGraph:dataModification => whenever data is
 * changed LineGraph:configModification => whenever config is changed
 */
function LineGraph(argsMap) {
	...
	/**
	 * This allows appending new data points to the end of the lines and sliding
	 * them within the time window: - x-axis will slide to new range - new data
	 * will be added to the end of the lines - equivalent number of data points
	 * will be removed from beginning of lines - lines will be transitioned
	 * through horizontoal slide to show progression over time
	 */
	this.slideData = function(newData) {
		...
		tempData.values.forEach(function(dataArrays, i) {
			...
			dataArrays.forEach(function(v) {
				...
				// shift the front value off to compensate for what we just
				// added
		 ...
		 * http://bost.ocks.org/mike/path/ In particular, view the HTML source
		 * for the last example on the page inside the tick() function.
		 */
		...
		// The quick-steps look cleaner, and keep the axis/line in-sync instead
		// of jittering
		redrawAxes(false);
		redrawLines(false);
	...
	/**
	 * This does a full refresh of the data: - x-axis will slide to new range -
	...
	this.updateData = function(newData) {
		...
		redrawAxes(true);
		// transition is 'false' for lines because the transition is really
		// weird when the data significantly changes
		// such as going from 700 points to 150 to 400
		// and because of that we rebind the data anyways which doesn't work
		// with transitions very well at all
		redrawLines(false);
	...
	// used to track if the user is interacting via mouse/finger instead of
	// trying to determine
	// by analyzing various element class names to see if they are visible or
	// not
	 ...
	 * 
	 * Expects a map like this: {"start": 1335035400000, "end": 1335294600000,
	 * "step": 300000, "values": [[28,22,45,65,34], [45,23,23,45,65]]}
	 */
	var processDataMap = function(dataMap) {
		// assign data values to plot over time
		var dataValues = getRequiredVar(dataMap, 'values',
		...
		var numAxisLabelsPowerScale = getOptionalVar(dataMap,
				'numAxisLabelsPowerScale', 6);
		var numAxisLabelsLinearScale = getOptionalVar(dataMap,
				'numAxisLabelsLinearScale', 6);
		...
		/*
		 * copy the dataValues array, do NOT assign the reference otherwise we
		 * modify the original source when we shift/push data
		 */
		var newDataValues = [];
	...
	var redrawAxes = function(withTransition) {
		...
		if (withTransition) {
			...
			graph.selectAll("g .x.axis").transition().duration(
					transitionDuration).ease("linear").call(xAxis)

			// slide y-axis to updated location
			graph.selectAll("g .y.axis.left").transition().duration(
					transitionDuration).ease("linear").call(yAxisLeft)

			if (yAxisRight != undefined) {
				// slide y-axis to updated location
				graph.selectAll("g .y.axis.right").transition().duration(
						transitionDuration).ease("linear").call(yAxisRight)
			}
		} else {
			// slide x-axis to updated location
	...
	var redrawLines = function(withTransition) {
		...
		// redraw lines
		if (withTransition) {
			graph.selectAll("g .lines path").transition().duration(
					transitionDuration).ease("linear").attr("d", lineFunction)
					.attr("transform", null);
		} else {
			graph.selectAll("g .lines path").attr("d", lineFunction).attr(
					"transform", null);
		}
	}

	/*
	 * Allow re-initializing the y function at any time. - it will properly
	 * determine what scale is being used based on last user choice (via public
	 * switchScale methods)
	 */
	var initY = function() {
		initYleft();
	...
	var initYleft = function() {
		...
		var numAxisLabels = 6;
		if (yScale == 'pow') {
			yLeft = d3.scale.pow().exponent(0.3).domain([ 0, maxYscaleLeft ])
					.range([ h, 0 ]).nice();
			...
			yLeft = d3.scale.log().domain([ 0.1, maxYscaleLeft ]).range(
					[ h, 0 ]).nice();
		} else if (yScale == 'linear') {
			yLeft = d3.scale.linear().domain([ 0, maxYscaleLeft ]).range(
					[ h, 0 ]).nice();
			numAxisLabels = data.numAxisLabelsLinearScale;
		}

		yAxisLeft = d3.svg.axis().scale(yLeft).ticks(numAxisLabels,
				tickFormatForLogScale).orient("left");
	}
	...
	var initYright = function() {
		...
		if (maxYscaleRight != undefined) {
			...
			var numAxisLabels = 6;
			if (yScale == 'pow') {
				yRight = d3.scale.pow().exponent(0.3).domain(
						[ 0, maxYscaleRight ]).range([ h, 0 ]).nice();
				numAxisLabels = data.numAxisLabelsPowerScale;
			} else if (yScale == 'log') {
				// we can't have 0 so will represent 0 with a very small number
				// 0.1 works to represent 0, 0.01 breaks the tickFormatter
				yRight = d3.scale.log().domain([ 0.1, maxYscaleRight ]).range(
						[ h, 0 ]).nice();
			} else if (yScale == 'linear') {
				yRight = d3.scale.linear().domain([ 0, maxYscaleRight ]).range(
						[ h, 0 ]).nice();
				numAxisLabels = data.numAxisLabelsLinearScale;
			}

			yAxisRight = d3.svg.axis().scale(yRight).ticks(numAxisLabels,
					tickFormatForLogScale).orient("right");
		}
	}

	/*
	 * Whenever we add/update data we want to re-calculate if the max Y scale
	 * has changed
	 */
	var calculateMaxY = function(data, whichAxis) {
		...
		// we get the max of the max of values for the given index since we
		// expect an array of arrays

		// we can shortcut to using data.maxValues since we've already
		// calculated the max of each series in processDataMap
		...
		// we now have the max values for the axis we're interested in so get
		// the max of them
	 ...
	 */
	var initX = function() {
		// X scale starts at epoch time 1335035400000, ends at 1335294600000
		// with 300s increments
		x = d3.time.scale().domain([ data.startTime, data.endTime ]).range(
				[ 0, w ]);
	...
	var createGraph = function() {
		...
		graph = d3.select("#" + containerId).append("svg:svg").attr("class",
				"line-graph").attr("width", w + margin[1] + margin[3]).attr(
				"height", h + margin[0] + margin[2]).append("svg:g").attr(
		...
		// y is all done in initY because we need to re-assign vars quite often
		// to change scales
		initY();

		// Add the y-axis to the left
		graph.append("svg:g").attr("class", "y axis left").attr("transform",
				"translate(-10,0)").call(yAxisLeft);

		if (yAxisRight != undefined) {
			// Add the y-axis to the right if we need one
			graph.append("svg:g").attr("class", "y axis right").attr(
					"transform", "translate(" + (w + 10) + ",0)").call(
					yAxisRight);
		}
		...
		.x(function(d, i) {
			/*
			 * Our x value is defined by time and since our data doesn't have
			 * per-metric timestamps we calculate time as (startTime + the step
			 * between metrics * the index)
			 * 
			 * We also reach out to the persisted 'data' object for time since
			 * the 'd' passed in here is one of the children, not the parent
			 * object
			 */
			...
			if (yScale == 'log' && d < 0.1) {
				// log scale can't have 0s, so we set it to the smallest value
				// we set on y
				d = 0.1;
			}

			/**
			 * This is a hack that relies on: a) the single-threaded nature of
			 * javascript that this will not be interleaved b) that lineFunction
			 * will always be passed the data[] for all lines in the same way
			 * each time
			 * 
			 * We then use an external variable to track each time we move from
			 * one series to the next so that we can have its seriesIndex to
			 * access information in the data[] object, particularly so we can
			 * determine what axis this data is supposed to be on.
			 * 
			 * I didn't want to split the line function into left and right
			 * lineFunctions as that would really complicate the data binding.
			 * 
			 * Also ... I can't figure out nested functions to keep it scoped so
			 * I had to put lineFunctionSeriesIndex as a variable in the same
			 * scope as lineFunction. Ugly. And worse ... reset it in
			 * redrawAxes.
			 * 
			 * Anyone reading this who knows a better solution please let me
			 * know.
			 */
			if (i == 0) {
		...
		lines = graph.append("svg:g").attr("class", "lines").selectAll("path")
		...
		// add a line group for each array of values (it will iterate the array
		// of arrays bound to the data function above)
		...
		// add a 'hover' line that we'll show as a user moves their mouse (or
		// finger)
		// so we can use it to show detailed values of each line
		hoverLineGroup = graph.append("svg:g").attr("class", "hover-line");
		// add the line to the group
		hoverLine = hoverLineGroup.append("svg:line").attr("x1", 10).attr("x2",
				10) // vertical line so same value on each
	...
	/**
	 * Create a legend that displays the name of each line with appropriate
	 * color coding and allows for showing the current value when doing a
	 * mouseOver
	 */
	var createLegend = function() {

		// append a group to contain all lines
		var legendLabelGroup = graph.append("svg:g").attr("class",
				"legend-group").selectAll("g").data(data.displayNames).enter()
				.append("g").attr("class", "legend-labels");

		legendLabelGroup.append("svg:text").attr("class", "legend name").text(
				function(d, i) {
					return d;
				}).attr("font-size", legendFontSize).attr("fill",
				function(d, i) {
		...
		// put in placeholders with 0 width that we'll populate and resize
		// dynamically
		legendLabelGroup.append("svg:text").attr("class", "legend value").attr(
				"font-size", legendFontSize).attr("fill", function(d, i) {
			return data.colors[i];
		}).attr("y", function(d, i) {
			return h + 28;
		})

		// x values are not defined here since those get dynamically calculated
		// when data is set in displayValueLabelsForPositionX()
	}

	var redrawLegendPosition = function(animate) {
		var legendText = graph.selectAll('g.legend-group text');
		if (animate) {
			legendText.transition().duration(transitionDuration).ease("linear")
					.attr("y", function(d, i) {
						return h + 28;
					});
	...
	var createScaleButtons = function() {
		...
		var buttonGroup = graph.append("svg:g").attr("class",
				"scale-button-group").selectAll("g").data(scales).enter()
				.append("g").attr("class", "scale-buttons").append("svg:text")
				...
				.attr("fill", function(d) {
					...
					// return it at the width of previous labels (where the last
					// one ends)
	...
	var handleMouseOverGraph = function(event) {
		...
		// debug("MouseOver graph [" + containerId + "] => x: " + mouseX + " y:
		// " + mouseY + " height: " + h + " event.clientY: " + event.clientY + "
		// offsetY: " + event.offsetY + " pageY: " + event.pageY + "
		// hoverLineYOffset: " + hoverLineYOffset)
		if (mouseX >= 0 && mouseX <= w && mouseY >= 0 && mouseY <= h) {
			...
			// proactively act as if we've left the area since we're out of the
			// bounds we want
			handleMouseOutGraph(event)
		}
	}
	...
	var handleMouseOutGraph = function(event) {
		...
		// debug("MouseOut graph [" + containerId + "] => " + mouseX + ", " +
		// mouseY)
	...
	/*
	 * // if we need to support older browsers without pageX/pageY we can use
	 * this var getMousePositionFromEvent = function(e, element) { var posx = 0;
	 * var posy = 0; if (!e) var e = window.event; if (e.pageX || e.pageY) {
	 * posx = e.pageX; posy = e.pageY; } else if (e.clientX || e.clientY) { posx =
	 * e.clientX + document.body.scrollLeft +
	 * document.documentElement.scrollLeft; posy = e.clientY +
	...
	var handleDataUpdate = function() {
		if (userCurrentlyInteracting) {
			// user is interacting, so let's update values to wherever the
			// mouse/finger is on the updated data
			if (currentUserPositionX > -1) {
				displayValueLabelsForPositionX(currentUserPositionX)
			}
		} else {
			// the user is not interacting with the graph, so we'll update the
			// labels to the latest
			setValueLabelsToLatest();
		}
	}
	...
	var displayValueLabelsForPositionX = function(xPosition, withTransition) {
		...
		var cumulativeWidth = 0;
		var labelNameEnd = [];
		graph.selectAll("text.legend.name").attr(
				"x",
				function(d, i) {
					// return it at the width of previous labels (where the last
					// one ends)
					var returnX = cumulativeWidth;
					// increment cumulative to include this one + the value
					// label at this index
					cumulativeWidth += this.getComputedTextLength() + 4
							+ labelValueWidths[i] + 8;
					// store where this ends
					labelNameEnd[i] = returnX + this.getComputedTextLength()
							+ 5;
					return returnX;
				})
		...
		if (cumulativeWidth > w) {
			...
			// debug("making legend fit by decreasing font size to: " +
			// legendFontSize)
			graph.selectAll("text.legend.name").attr("font-size",
					legendFontSize);
			graph.selectAll("text.legend.value").attr("font-size",
					legendFontSize);
		...
		// show the date
		graph.select('text.date-label').text(
				dateToShow.toDateString() + " "
						+ dateToShow.toLocaleTimeString())

		// move the group of labels to the right side
		if (animate) {
			graph.selectAll("g.legend-group g").transition().duration(
					transitionDuration).ease("linear").attr("transform",
					"translate(" + (w - cumulativeWidth) + ",0)")
		} else {
			graph.selectAll("g.legend-group g").attr("transform",
					"translate(" + (w - cumulativeWidth) + ",0)")
		}
	}
	...
	/**
	 * Convert back from an X position on the graph to a data value from the
	 * given array (one of the lines) Return {value: value, date, date}
	 */
	var getValueForPositionXFromData = function(xPosition, dataSeriesIndex) {
		...
		// The date we're given is interpolated so we have to round off to get
		// the nearest
		// index in the data array for the xValue we're given.
		// Once we have the index, we then retrieve the data from the d[] array
		index = Math.round(index);

		// bucketDate is the date rounded to the correct 'step' instead of
		// interpolated
	...
	var handleWindowResizeEvent = function() {
		...
		d3.select("#" + containerId + " svg").attr("width",
				w + margin[1] + margin[3]).attr("height",
				h + margin[0] + margin[2]);
		...
		if (yAxisRight != undefined) {
			...
			graph.selectAll("g .y.axis.right").attr("transform",
					"translate(" + (w + 10) + ",0)");
		}

		// reset legendFontSize on window resize so it has a chance to
		// re-calculate to a bigger size if it can now fit
		legendFontSize = 12;
		// debug("making legend fit by decreasing font size to: " +
		// legendFontSize)
		...
		// move date label
		graph.select('text.date-label').transition().duration(
				transitionDuration).ease("linear").attr("x", w)
	...
	/*
	 * round a number to X digits: num => the number to round, dec => the number
	 * of decimals
	 */
	/* private */
	function roundNumber(num, dec) {
		...
		if (dec > 0) {
			...
			// make sure we have a decimal and pad with 0s to match the number
			// we were asked for
